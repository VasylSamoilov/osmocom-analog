...
/* ===== TEST CORRUPTION FUNCTIONS (remove after testing) =====
 *
 * These functions inject controlled bit errors into the TX output
 * to verify BCH correction and S2 timing recovery.
 *
 * Enable by setting flex_test_corruption = 1 (e.g. via debugger or
 * by changing the initializer below).  Set to 0 to disable.
 */
static int flex_test_corruption = 0;

/* Flip a single bit in a byte buffer.
 * bit_pos is the bit index (0 = MSB of byte 0). */
static void flip_bit(uint8_t *buf, int bit_pos)
{
	buf[bit_pos / 8] ^= (0x80U >> (bit_pos % 8));
}

/* Flip n_bits random distinct bits in buf[0..total_bits-1].
 * n_bits must be <= total_bits. */
static void flip_random_bits(uint8_t *buf, int total_bits, int n_bits)
{
	int positions[3];
	int i, j, pos;

	for (i = 0; i < n_bits && i < 3; i++) {
...
/* Corrupt S1 portion of sync_buffer: flip n_bits random bits.
 * S1 = first 14 bytes = 112 bits (BS1 + A + B + inv.A).
 * FIW (last 4 bytes) is left untouched. */
static void test_corrupt_s1(uint8_t *sync_buffer, int n_bits)
{
	if (n_bits < 1 || n_bits > 3)
		return;
	fprintf(stderr, "TEST: corrupting S1 with %d bit flip%s\n",
		n_bits, n_bits == 1 ? "" : "s");
	flip_random_bits(sync_buffer, 112, n_bits);
}

/* Corrupt S2 portion of frame_buffer: flip n_bits random bits.
 * S2 is the first s2_bytes of frame_buffer.
 * s2_bytes: 5 (1600), 10 (3200), 20 (6400). */
static void test_corrupt_s2_bits(uint8_t *frame_buffer, int s2_bytes, int n_bits)
{
	if (n_bits < 1 || n_bits > 3 || s2_bytes <= 0)
		return;
	fprintf(stderr, "TEST: corrupting S2 with %d bit flip%s\n",
		n_bits, n_bits == 1 ? "" : "s");
	flip_random_bits(frame_buffer, s2_bytes * 8, n_bits);
}

/* Shift S2 timing by shifting the entire frame_buffer by +1 or -1 bit.
 *   shift = +1: prepend one zero bit (delays S2 by 1 bit = late boundary)
 *   shift = -1: remove first bit (advances S2 by 1 bit = early boundary)
 *   shift =  0: no change */
/* Shift the entire frame_buffer by N bits right (positive) or left (negative).
 * Used by test_corrupt_s2_timing to shift by ±1 symbol worth of bits. */
static void shift_buffer_bits(uint8_t *buf, int *len, int buf_size, int nbits)
{
	int n = (nbits > 0) ? nbits : -nbits;
	int l = *len;
	int i, b;
	...
	if (nbits > 0) {
		/* Shift right by n bits (prepend n zero bits = late) */
		int new_len = (l * 8 + n + 7) / 8;
		if (new_len > buf_size)
			return;
		/* Pad tail with zero bytes if buffer grows */
		for (i = l; i < new_len; i++)
			buf[i] = 0;
		/* Shift right one bit at a time */
		for (b = 0; b < n; b++) {
			for (i = new_len - 1; i > 0; i--)
				buf[i] = (buf[i - 1] << 7) | (buf[i] >> 1);
			buf[0] >>= 1;
		}
		*len = new_len;
	} else {
		/* Shift left by n bits (remove n leading bits = early) */
		for (b = 0; b < n; b++) {
			for (i = 0; i < l - 1; i++)
...
/* Shift S2 timing by ±1 symbol.
 *   shift = +1: prepend 1 symbol worth of zero bits (late boundary)
 *   shift = -1: remove 1 symbol worth of leading bits (early boundary)
 *   shift =  0: no change
 * bits_per_symbol: 1 for 2FSK, 2 for 4FSK. */
static void test_corrupt_s2_timing(uint8_t *frame_buffer, int *frame_length,
				   int buf_size, int shift, int bits_per_symbol)
{
	if (shift == 0 || *frame_length <= 0)
		return;

	int nbits = shift * bits_per_symbol;
	fprintf(stderr, "TEST: shifting S2 timing %+d symbol (%+d bits, %dFSK)\n",
		shift, nbits, bits_per_symbol == 2 ? 4 : 2);
	shift_buffer_bits(frame_buffer, frame_length, buf_size, nbits);
}

/* Compute S2 byte size from bitrate (mirrors flex_s2_size in frame.c). */
static int test_s2_bytes(int bitrate)
{
	int bs2;
	switch (bitrate) {
	case 1600: bs2 = 4;  break;
	case 3200: bs2 = 24; break;
	case 6400: bs2 = 64; break;
	default:   return 0;
	}
	return (2 * bs2 + 32 + 7) / 8;
}
/* ===== END TEST CORRUPTION FUNCTIONS ===== */
				     ...
				     int *msgs_packed, int *error)
{
	...
	/* TEST: inject controlled corruption if enabled */
	if (flex_test_corruption && sync_len > 0 && data_len > 0) {
		static int seeded = 0;
		int s1_flips, s2_flips, s2_shift, s2b;
		...
		s1_flips = (rand() % 3) + 1;		/* 1, 2, or 3 */
		s2_flips = (rand() % 3) + 1;		/* 1, 2, or 3 */
		s2_shift = (rand() % 3) - 1;		/* -1, 0, or +1 */
		s2b = test_s2_bytes(params->bitrate);

		test_corrupt_s1(flex->sync_buffer, s1_flips);
		test_corrupt_s2_bits(flex->frame_buffer, s2b, s2_flips);
		test_corrupt_s2_timing(flex->frame_buffer,
				       &flex->frame_buffer_length,
				       (int)sizeof(flex->frame_buffer),
				       s2_shift,
				       (params->modulation_type == FLEX_MOD_4FSK) ? 2 : 1);
	}
}
